#!/usr/bin/env python3
"""Append completed months to data/iex-notional.json from Cboe's market share pages.
...
matches it exactly. Existing months are preserved; only months after the
saved series are fetched.
...
def main():
    ...
    start_y, start_m = map(int, data["start"].split("-"))
    values = data["values"]
    next_y, next_m = divmod(start_y * 12 + start_m - 1 + len(values), 12)
    next_m += 1
    if (next_y, next_m) > (end_y, end_m):
        print("IEX notional data is already up to date")
        return

    for y, m in month_range(next_y, next_m, end_y, end_m):
